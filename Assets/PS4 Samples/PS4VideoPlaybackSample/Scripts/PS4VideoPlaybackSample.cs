...
public class PS4VideoPlaybackSample : MonoBehaviour
{
    ...
    void CropVideo()
    {
        ...
        if (videoImage != null)
        {
            ...
            if ((width > 0) && (height > 0))
            {
                int videoWidth = width - cropLeft - cropRight;
                scalex = (float)videoWidth / (float)width;
                offx = (float)cropLeft / (float)width;
                int videoHeight = height - cropTop - cropBottom;
                scaley = (float)videoHeight / (float)height;
                offy = (float)cropBottom / (float)height;
            }

            // Typically we want to invert the Y on the video because thats how planes UV's are laid out
