...
public class ExportPS4DRMPackages
{
	static string serviceID = "ED1633-NPXX51362_00";				// Must match the serviceID of the application that the content will be used with.
	...
	{
		public DRMPackageDesc(string title, string label, string entitlementkey, string[] bundlePaths, string[] rawPaths, bool free)
		{
			contentTitle = title;
			contentLabel = label;
		...
		public string entitlementKey;		// additional content package, entitlementKey
	...
	{
		new DRMPackageDesc("Rock1", "ROCK100000000000", "00112233445566778899aabbccdd0000", new string[] { "Assets/DRMContent/Rock1.prefab" }, new string[] { "Assets/DRMContent/Rock1_desc.txt" }, false ),
		new DRMPackageDesc("Rock2", "ROCK200000000000", "00112233445566778899aabbccdd0001", new string[] { "Assets/DRMContent/Rock2.prefab" }, new string[] { "Assets/DRMContent/Rock2_desc.txt" }, false ),
	};
	
	[MenuItem("PS4 Tools/Export DRM Content Packages")]
	static void ExportPS4DRMContentPackages()
	{
		if (!PS4SDKTools.CheckSDKToolsExist(false))
		{
			Debug.LogError("PS4 SDK tools not found\n");
			return;
		}
		...
		foreach(DRMPackageDesc desc in packages)
		{
			...
			CreateDRMPackage(desc.contentTitle, contentID, desc.entitlementKey, passcode, desc.isFree, bundles, assets, packagePath);
		}
	}
	
	[MenuItem("PS4 Tools/Install DRM Content Package")]
	...
	//
	static void CreateDRMPackage(string contentTitle, string contentID, string entitlementKey, string passcode, bool isFree, string[] bundles, string[] rawAssets, string packagePath)
	{
		...
		{
			System.Console.WriteLine("adding " + file);
			fileNames.Add(file);
		}
		...
		// Build the package
		if (!BuildPackage(stagingArea, fileNames.ToArray(), contentID, entitlementKey, passcode, isFree, packagePath))
		{
	...
	static bool BuildParamFile(string contentTitle, string contentID, string paramSfx, string paramSfo)
	{
		...
		if (!PS4SDKTools.ValidateContentID(contentID))
		{
		...
		// from Param_File_Editor-Users_Guide_e.pdf
		sfxParams.Set("FORMAT", "obs");
		...
		psi.FileName = PS4SDKTools.GetTool("orbis-pub-cmd");
		psi.UseShellExecute = false;
		psi.CreateNoWindow = true;
		psi.WorkingDirectory = Directory.GetCurrentDirectory();
		psi.RedirectStandardError = true;

		if(!PS4SDKTools.RunCommand2(psi, string.Format("sfo_create \"{0}\" \"{1}\"", paramSfx, paramSfo), errors, null))
		{
	...
	static bool BuildPackage(string stagingArea, string[] files, string contentID, string entitlementKey, string passcode, bool isFree, string packagePath)
	{
		var psi = new System.Diagnostics.ProcessStartInfo();
		psi.FileName = PS4SDKTools.GetTool("orbis-pub-cmd");
		psi.UseShellExecute = false;
		...
		string project = Path.Combine(stagingArea, "project.gp4");
//		string masterVersion = "01.00";
		string projectType = "pkg_ps4_ac_data";
		StringBuilder errors = new System.Text.StringBuilder("");
		...
		if(!PS4SDKTools.RunCommand2(psi, string.Format("gp4_proj_create --volume_type {0} --content_id {1} --entitlement_key {2} --passcode {3} {4}",
						projectType, contentID, entitlementKey, passcode, project), errors, null))
		{
		...
		{
			string strippedFile = file.Substring(stagingArea.Length +1);
			if(!PS4SDKTools.RunCommand2(psi, string.Format("gp4_file_add \"{0}\" {1} {2}", file, strippedFile, project), errors, null))
			{
		...
		// Build the package.
		if (!PS4SDKTools.RunCommand2(psi, string.Format("img_create \"{0}\" \"{1}\"", project, package), errors, null))
		{
	...
	static void InstallDRMPackage(string packageName)
	{
		StringBuilder errors = new System.Text.StringBuilder("");
		var psi = new System.Diagnostics.ProcessStartInfo();
		psi.FileName = PS4SDKTools.GetTool("orbis-ctrl");
		psi.UseShellExecute = false;
		psi.CreateNoWindow = true;
		psi.WorkingDirectory = Directory.GetCurrentDirectory();
		psi.RedirectStandardError = true;
		EditorUtility.DisplayProgressBar("Installing DRM Package", "", 0.5F);
		if (!PS4SDKTools.RunCommand2(psi, string.Format("pkg-install \"{0}\"", packageName), errors, null))
		{
